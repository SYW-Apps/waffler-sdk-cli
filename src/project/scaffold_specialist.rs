...
fn render_manifest(fqid: &str, version: &str, description: &str) -> String {
  ...
  "//middleware": "Interceptors core runs around routed bus calls: {{ \"id\": \"auth\", \"handler\": \"my.pkg.intercept\", \"needsHeaders\": true, \"scope\": {{ \"commands\": {{ \"targets\": {{ \"any_of\": [\"db\", \"syw.system.*\"] }} }} }} }}. `handler` is the capability IN THIS PACKAGE the host invokes for each matched envelope -- distinct from `scope.commands.capabilities`, which is the capability being INTERCEPTED. \"kind\" is \"Enforcing\" (the default: its verdict can reject or patch a message) or \"Observing\" (a witness that can do neither), spelled exactly -- a misspelling is refused, never read as the default. \"required\" defaults to true: a required interceptor that is denied stops this package enabling, an optional one is skipped and reported. ANY OTHER KEY on a declaration is REFUSED by name, including the ones the node stamps itself (owner, consentDigest). The scope is core's own type, so its fields stay snake_case while the rest of this file is camelCase: one definition of the matcher rather than two that can drift. NOTHING IS ACQUIRED BY OMISSION -- a block that narrows nothing and does not say \"everything\": true is REFUSED, here and at the node, so the broadest scope cannot be the emptiest-looking declaration. COMMANDS AND EVENTS ARE SEPARATE BLOCKS: knowing which events a service listens to is enough to manipulate it, so event interception is never acquired by leaving a topic list empty in a command block -- you have to write an \"events\" block. An exclusion ALONE is not a narrowing: \"everything except db\" still reaches every service anyone adds tomorrow. Declare the NARROWEST scope that is still correct -- core assembles the chain in-process, so a call your scope excludes never crosses into this package at all. THE REGISTRATION GRANT IS THE HOST'S, NOT YOURS, on waffler_core 78b3acf8 and later: approving each middleware declaration is the one door, and the host requests `bus:register_middleware` for you in the reserved `host.` group namespace. Only a node older than that needs a permission group of yours requesting it -- and then scope that group's bus rule to the `register_middleware` command rather than to every bus verb.",
  "middleware": [],

  "//permissionGroups": "What this package asks an operator to grant, in security's own group schema, passed through as written. A group id under `host.` is REFUSED here and at the node: that namespace belongs to the host, which synthesizes groups there itself.",
  "permissionGroups": [],
  "fastLaneRequests": [],
  "uiPlugins": []
}}
