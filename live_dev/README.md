...
| `02-install.py` | a second registry is added **alongside** the default; the node reaches it; search, detail, plan and preview all resolve; **install succeeds**, the package is then **enabled** (installing is not enabling) and only then asked to answer. It clears any copy the node already holds first: installing onto a mounted package moves the record and not the actor (D14/D15) |
| `03-grant-and-call.py` | with no rule the call is refused `AccessDenied`, **naming the caller as `syw.app.web`** rather than only its fingerprint; a permission group + binding changes that to `NotFound` — the grant is what moved it. Takes `CYCLE_FQID`, and REFUSES TO START rather than reporting failures when its preconditions are not met. It removes the group and binding it creates BEFORE the baseline call, so a second run does not read its own leftover grant as the enforcer having failed, and it enables the package it reinstalls |
...
| `08-login.sh` | the interactive login, against a real provider: PKCE S256, a state parameter, an ephemeral loopback port; the credential is persisted **per registry** and does **not** leak to another; and a publish succeeds on the browser-obtained credential with **no environment token set**. It bumps the code's VERSION constant with the manifest and withdraws any 0.1.1 an earlier run left, because a manifest-only bump ships an artifact that answers with the old version forever |
...
| `12-install-closure.py` | installs that closure through the marketplace on the live node: all three land, **in order**, and **at the version the registry offered** — a stale row would otherwise read as success. Both kinds of unresolvable closure — a dependency that does not exist, and one no version satisfies — are **refused with nothing installed**, naming what disagrees; a repeat install is **not a second copy**. It clears the closure first and then **enables in plan order**, which is where the order claim now lives: an enable is refused while a required dependency is not running, so a closure delivered in the wrong order fails on the first enable that needs one |
| `13-closure-serves.py` | after a restart, **every member of the closure answers** — not just the root — identifies itself, and reports the version the node records. Then it takes the leaf away and reports what a single process can see of that. It enables the closure after the reinstall, and its version comparison is RED against D15 until the node restarts: a reinstalled package serves the old image |
...
| `20-reserved-group-publish.sh` | a bundle from ANOTHER PACKER declaring a permission group under `host.`: an ordinary CLI pack with the group added to `.manifest` afterwards, every other entry byte-identical (the leg checks its own doctoring first). `waffler validate` refuses the same declaration; **the registry refuses the doctored bundle, naming the reserved namespace, and does not list it (404)**; and the same bundle WITHOUT the group publishes at the same version (200) — the control that attributes the refusal to the group rather than the namespace, version or upload. Needs a registry built with the `waffler_shared` git dependency (`WAFFLER_SHARED_TOKEN`) |
| `21-approval-door-publish.sh` | publishes `devbot.mwa.gate`: one REQUIRED permission group, one fast-lane request and one REQUIRED middleware declaration (`Observing`, scoped to commands addressed to itself — an approved declaration joins the GLOBAL bus chain, so a fixture that could reject messages is a fixture that can wedge a shared node). Asserts the catalog carries all three asks with the middleware SCOPE intact — the registry used to read the declarations only to set a `declares_middleware` boolean, so a package page could show two thirds of what a package wants; an absent key is failed separately from an empty list, because a registry that cannot say is not a package that declares none. It deliberately does NOT ask for `bus:register_middleware`: the CLI advises against it and is right, because this core grants that verb through the host's own `host.middleware_grant` |
| `22-approval-door.py` | the POST-INSTALL approval door, on the live node. The preview carries the middleware row — kind, scope and the DIGEST an approval binds to — beside the group and the lane; an **Automated install commits with all three unanswered** (no approved group, no lane grant, a Pending review already bound to the previewed digest, and the host's grant group present but unapproved); **enable refuses `PermissionUnapproved`**, and after approving only the group **refuses again as `MiddlewareUnapproved`** — two separate doors; a declaration the package never made is refused `NotFound` and changes nothing; approving the lane and the declaration records the review bound to the digest the operator SAW, materializes the lane grant and flips `host.middleware_grant` in the SECURITY CATALOG (read where it lives, not as a record field, and always scoped by `source` — a bare lookup answers with some other package's copy); and only then does enable succeed |
