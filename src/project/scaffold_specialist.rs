...
fn render_manifest(fqid: &str, version: &str, description: &str) -> String {
  ...
  "//middleware": "Interceptors core runs around routed bus calls: {{ \"id\": \"auth\", \"handler\": \"my.pkg.intercept\", \"target\": \"syw.system.*\", \"needsHeaders\": true, \"filters\": {{ \"capability\": \"admin.*\" }} }}. `handler` is the capability IN THIS PACKAGE the host invokes for each matched envelope -- not to be confused with `filters.capability`, which is the capability being INTERCEPTED. `target` absent intercepts EVERY routed call. Declare the NARROWEST filters that are still correct: core assembles the chain in-process, so a call your filters exclude never crosses into this package at all -- filtering is how an interceptor avoids being asked about traffic it would only wave through. Three keys are refused inside `filters`: `syw.owner` and `syw.fqid` (a node stamps the declaring package identity into them at registration) and `target` (it has its own field). `source` IS allowed -- the global bus chain reads it as a filter on the envelope source. AND DECLARING ANY MIDDLEWARE COMMITS YOU TO A GRANT: core registers each one on the global bus chain, that is gated on `bus:register_middleware`, and failing it is FATAL -- so a package declaring middleware without a permission group requesting that grant will not run.",
  "middleware": [],
  "permissionGroups": [],
  "fastLaneRequests": [],
  "uiPlugins": []
